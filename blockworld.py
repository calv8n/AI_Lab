            ...
            def select_move(self):
                for index, stack in enumerate(self.first_stack):
                    for index2, stack2 in enumerate(self.first_stack):
                        if index != index2:
                            curr_table, move = self.valid_state_move(
                                self.first_stack, index, index2
                            )
                            new_state = State(
                                curr_table,
                                self.second_stack,
...
if __name__ == "__main__":
    ...
    initial_arrangement = [["A", "B", "C"]]
    goal_arrangement = [["C", "B", "A"]]
    print(test_agent.solve(initial_arrangement, goal_arrangement))
